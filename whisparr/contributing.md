...
Whisparr is written in C# (backend) and JS (frontend). The backend is built on the .NET 8 framework, while the frontend utilizes Reactjs.
...
> Whisparr will **NOT** run on older versions such as `10.x`, `8.x`, `6.x`, or any version below 12.0!
{.is-warning}
...
> Ensure startup project is set to `Whisparr.Console` and framework to `net8.0`
{.is-info}
