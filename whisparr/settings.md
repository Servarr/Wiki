...
- Once you've clicked the `+` button to add a new indexer you will be presented with a new window with many different options. For the purposes of this wiki Whisparr considers both Usenet Indexers and Torrent Trackers to be "Indexers".
...
- One of the best and simplest ways to utilize Torrent trackers with Whisparr is to utilize a second program such as [Prowlarr](/prowlarr) or [Jackett](https://github.com/Jackett/Jackett). These software pair well with Whisparr as a search indexer that houses all your information and sends it to Whisparr.
...
- If the "Completed Download Handling - Remove" option is enabled in Whisparr's settings, Whisparr will delete the original file and torrent from your client, but only if the client reports that seeding is complete and torrent is stopped.
...
- A list of supported notifications & connections is located [here](/whisparr/supported#notifications)
