...
The log files are located in 's [Appdata Directory](/sonarr/appdata-directory), inside the logs/ folder. You can also access the log files from the  UI at System -> Logs -> Files.
...
The update log files are located in 's [Appdata Directory](/sonarr/appdata-directory), inside the UpdateLogs/ folder.
...
The best place to look when  won't start after an update is your log files, before trying to start  again, use [Logging](/sonarr/settings#logging) and [Log Files](/sonarr/system#log-files) to find them and increase the log level.
