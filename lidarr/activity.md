...
> For Usenet clients, Lidarr only looks 60 items deep in the queue for potential imports. If your queue exceeds this depth, items won't be picked up and will require manual imports.
{.is-warning}
...
| Grey clock | Release Pending | The download is waiting for a Delay Profile timer to expire before being sent to the download client. | Wait for the delay to pass, or adjust your Delay Profile in Settings. |
...
The icon on the left of each row shows the action that was taken. Use the **Filter** button to narrow the view by event type, and **Options** to show or hide columns.
