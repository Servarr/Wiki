...
Prowlarr stores its data in an SQLite database. The SQLite3 library installed on your system is too old. Prowlarr requires at least version 3.9.0. Note that Prowlarr uses [libSQLite3.so](http://libSQLite3.so) which may or may not be contained in a SQLite3 upgrade package.
