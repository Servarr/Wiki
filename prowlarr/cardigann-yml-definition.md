...
- oneurl: The value of the `oneurl` input is appended directly to `path` (no `?` is added) and the result is requested with HTTP GET. No other inputs are sent.
...
| Key | Type | Default | Description |
| --- | --- | --- | --- |
| `method` | string | `form` | `form`, `post`, `get`, `cookie` or `oneurl`. See [Login](#login). |
| `path` | string | none | Login page (`form`) or login target (`post`, `get`, `oneurl`). Templates are applied. |
...
| `inputs` | map | none | Values to send. Templates are applied. For `form`, they override the values found in the form. Not used by `cookie`. For `oneurl`, only the `oneurl` input is used. |
