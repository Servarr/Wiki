...
So pick *one* path layout and use it for all of them. I'm a fan of
...
`-v /host/data/torrents:/data/torrents`. In the torrent software
settings, you'll need to reconfigure paths and you can sort into
subfolders like`/data/torrents/{tv|movies|music}`.
...
`-v /host/data/usenet:/data/usenet`. In the usenet software settings,
...
`-v /host/data/media:/data/media`, which can have any number of
subfolders like `movies`, `kids movies`, `tv`, `documentary tv` and/or
`music` as sub folders.
...
The biggest is that volumes defined in the `Dockerfile` will get created
...
-   [hotio's](https://hub.docker.com/u/hotio)
    ...
    [nzbhydra2](https://hub.docker.com/r/hotio/nzbhydra2) and
...
example, if you
`docker run --network=isolated --hostname=deluge binhex/arch-deluge` and
`docker run --network=isolated --hostname=radarr binhex/arch-radarr`,
you can then configure the Download Client in Radarr to point at just
`deluge` and it'll work *and* communicate on its own private network.
Which means if you wanted to be even more secure, you could *stop*
forwarding that port too. If you put your reverse proxy container on the
same network, you can even stop forwarding the web interface ports and
