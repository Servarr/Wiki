...
your system to allow your Arr's and media serving apps to access the
data securely is possible. For this guide we will use [Google Suite
Unlimited Storage](https://gsuite.google.com/products/drive/) or a
...
3.  Under "ENABLE APIS AND SERVICES" search for "Google Drive", and
    ...
    SERVICES](ENABLE_APIS_AND_SERVICES.png "ENABLE APIS AND SERVICES")
4.  Click "Credentials" in the left-side panel, then "Create
    Credentials" in the upper right. ![Create New
    Credentials](Google_Drive_Credentials_Screenshot.png
...
3.  Use the "Create Credentials" button. Fill in "Service account name"
    with something that identifies your client. e.g. `mount`Leave "Role"
    Empty
4.  Tick "Furnish a new private key" - select "Key type JSON".
5.  Tick "Enable G Suite Domain-wide Delegation". These credentials are
    what rclone will use for authentication. If you ever need to remove
    access, press the "Delete service account key" button.
...
/directory/` will show the permissions for that folder. In the example
below, the user and group are both `plex`. Example:
...
This example unit assumes that the `User` and `Group` are both `plex`,
Mergerfs's executable is placed in `/usr/local/bin/`, your local storage
is mounted at `/mnt/local`, your cloud storage is mounted at
...
4.  The SQLLite prompt will open up. Type this next: `PRAGMA
    default_cache_size = 6000000;` and don't forget the trailing
    semicolon.
